...
void init(int argc, char **argv)
{
    ...
    //ÄÚºËÊ±ÇøÉèÖÃº¯Êý
	tzset();
    //µ÷ÕûÏµÍ³Ê±¼ä£¬Õâ¸öºÍºóÐøÅÐ¶ÏÁ´½Ó³¬Ê±ÓÐ¹ØÏµ¡£
	tv_update_date(-1,-1);
	start_date = now;

    //²úÉúËæ»úÊýÖÖ×Ó£¬ºóÐøÓÉrandomº¯Êý²úÉúËæ»úÊý
	srandom(now_ms - getpid());

    //ÉèÖÃÎÄ¼þÃèÊö·û¼¯fdsetÖÐ¶ÔÓ¦ÎÄ¼þÃèÊö·ûfdµÄÎ»£¬±íÃ÷ÄÄÐ©ÐèÒª±»ºöÂÔ
    //Õâ¸öºÍsyslogÓÐ¹Ø
	init_log();

	//ÔÝÊ±Àí½âÎªºÍÐÅºÅ´¦ÀíÓÐ¹ØµÄº¯Êý¡£Èç¹ûÕë¶ÔÓÚhaproxyµÄºËÐÄ´úÂë£¬ÔÝÊ±¿ÉÒÔºöÂÔ¡£
	signal_init();

	//³õÊ¼»¯ACL¹æÔò£¬¼ì²éACL¹æÔòÊÇ·ñÔÚsample_listÖÐ×¢²á¡£Õâ¸ösample_listÊÇÄÚºËÏà¹ØµÄ?
	if (init_acl() != 0)
		exit(1);

	//´´½¨Ò»¸öÈÎÎñ³Ø£¬»òÕß¸´ÓÃÒ»¸öÈÎÎñ³Ø¡£poolÊÇÒ»¸öpool_head½á¹¹µÄÊý×é¡£
	...
	//´¦ÀíhttpÐ­ÒéµÄ³õÊ¼»¯£¬²¢´´½¨http_txn,requri,uniqueidµÄpool
	init_proto_http();
    ...
    //¸ù¾ÝÆô¶¯²ÎÊýÉèÖÃÏàÓ¦µÄÈ«¾Ö±äÁ¿¡£ÒòÎª°üÀ¨¶ÁÈ¡ÅäÖÃ£¬¶¼ÊÇÔÚÕâÒ»²½·¢Éú£¬
    ...
    //ÉèÖÃÄ£Ê½¡£µ±³öÏÖ¶àÖÖÄ£Ê½Ê±£¬½øÐÐÑ¡Ôñ¡£
    ...
    //¼ì²éÅäÖÃÎÄ¼þÊÇ·ñÅäÖÃÁË£¬·ñÔòµ÷ÓÃusageÌáÊ¾haproxyµÄÓÃ·¨£¬²¢ÍË³ö¡£
...
void run_poll_loop()
{
	...
	while (1) {
		/* Process a few tasks */
		//´¦Àí¿ÉÔËÐÐµÄÈÎÎñ¡£
		//ÕâÐ©ÈÎÎñÏ£ÍûÏÂ´ÎÔËÐÐ¾¡¿ìµÄ±»Ö´ÐÐ¡£¶ÔÓÚtcpºÍhttpÒµÎñÁ÷Á¿µÄ´¦Àí£¬¸Ãº¯Êý×îÖÕµ÷ÓÃprocess_sessionÍê³É£¬°üÀ¨½âÎöÒÑ¾­½ÓÊÕµ½µÄÊý¾Ý£¬
		//²¢Ö´ÐÐÒ»ÏµÁÐload balanceÌØÐÔ,µ«ÊÇ²»¸ºÔð´ÓsocketÊ×·¢Êý¾Ý¡£
		process_runnable_tasks();

		/* check if we caught some signals and process them */
		//²¶×½ÐÅºÅ£¬ÕâÐ©ÐÅºÅ¿ÉÄÜÊÇ¼üÅÌµÄctr+\ÃüÁî£¬ÏàÓ¦µÄÖ´ÐÐÍË³öÃüÁî¡£
		signal_process_queue();

		/* Check if we can expire some tasks */
		//»½ÐÑÄÇÐ©±»·ÅÈëwait queueÖÐµÄÈÎÎñ£¬½«Æä·ÅÈërun queueÖÐ¡£
		...
		//²¢×¢²á¶ÔÓ¦pollerµÄpoll·½·¨¡£ÀýÈçepollµÄÏà¹Øº¯Êý½Ó¿ÚÔÚev_epoll.cÖÐ¡£
		//ÕâÀï¾ÍÊÇÖ´ÐÐÒÑ¾­×¢²áµÄpollerµÄpoll·½·¨£¬Ö÷Òª¹¦ÄÜÊÇ»ñÈ¡ËùÓÐ»î¶¯µÄfd£¬²¢µ÷ÓÃ¶ÔÓ¦µÄhandler£¬Íê³É½ÓÊÜÐÂ½¨Á¬½Ó¡¢
		//Êý¾ÝÊÕ·¢µÈ¹¦ÄÜ¡£
		cur_poller.poll(&cur_poller, next);

		//pollerµÄpoll·½·¨Ö´ÐÐÊ±£¬³ÌÐò»á½«Ä³Ð©·ûºÏÌõ¼þÒÔ±ãÔÙ´ÎÖ´ÐÐIO´¦ÀíµÄfd·Åµ½fd_spec list[]ÖÐ£¬fd_process_cached_events»áÔÙ´ÎÖ´ÐÐÕâÐ©¡£
...
//argcÊÇÔËÐÐåhaproxy´«µÝ½øÀ´µÄ²ÎÊý¸öÊý£¬argvÊÇÔËÐÐhaproxyÊ±´«µÝ½øÀ´µÄ¾ßÌå²ÎÊý¡£
int main(int argc, char **argv)  
{
    ...
    //¸ù¾ÝÃüÁîÐÐ´«½øÀ´µÄ²ÎÊý½øÐÐ³õÊ¼»¯¡£¸Ãº¯ÊýÎ´·ÖÎöÍê³É¡£
    init(argc, argv);     


	//²¶»ñ¶à¸öÐÅºÅ£¬ÕâÐ©ÐÅºÅÊÇÓÉÄÚºËÌá¹©µÄ¡£·Ö±ðÊ±å£º
	/*
    SIGQUIT:¼üÅÌ°´ÏÂCtr+\
    SIGUSR1:ÓÃ»§×Ô¶¨ÒåµÄÐÅºÅ
    SIGHUP:¹ÒÆðÐÅºÅ¡£ÀýÈçÖÕ¶Ë¹Ø±ÕÊ±¡£
    SIGPIPE:¶ÔÒÑ¾­¹Ø±ÕµÄsocket½øÐÐÐ´²Ù×÷´¥·¢µÄÐÅºÅ¡£
    Õë¶Ô²»Í¬µÄÐÅºÅ£¬»áµ÷ÓÃ²»Í¬µÄº¯Êý¡£ÀýÈçSIGQUIT»á´¥·¢dumpº¯Êý¡£
	*/
	...
	//µ÷Õû×ÊÔ´ÉÏÏÞ
	/*
    Ö±½ÓÍ¨¹ýÏµÍ³µ÷ÓÃ½øÐÐÉèÖÃ¡£±ÈÈçº¯Êýsetrlimit¡£
	*/
	/* ulimits */
	...
	//Ä¿Ç°µÄÀí½âÊÇ£¬ÕâÐ©proxies¾ÍÊÇÐèÒª¼àÌýµÄ¶Ë¿Ú¡£°üÀ¨¹ÜÀí½çÃæºÍ´úÀí¶Ë¿Ú¡£
    ...
    //°ó¶¨ËùÓÐ×¢²áµÄÐ­Òé¡£ÕâÀïµÄÐ­ÒéÊÇ°üÀ¨tcp,udp,httpµÄÂð?
    ...
    //×¢²áÐÅºÅÐèÒªµ÷ÓÃµÄº¯Êý¡£ÎªÊ²Ã´ºÍ¿ªÊ¼µÄ4¸öÐÅºÅ²»Ò»Í¬×¢²á£¬»¹Ã»ÓÐ·ÖÎö¡£
	...
	//ÉèÖÃ°²¾²µÄÆô¶¯Ä£Ê½¡£´ËÊ±Æô¶¯Ê±ÐèÒªÐ¶ÔØtty¡£
    ...
    //chroot±£»¤½ø³ÌÖ»ÄÜÔÚÖ¸¶¨µÄÄ¿Â¼ÏÂ»î¶¯£¬¶øÎÞ·¨²Ù×÷ÏµÍ³ÏµÍ³Ä¿Â¼¡£ÎªÁË°²È«ÐÔ£¬Õâ¸öÒ»°ã¶¼ÒªÉèÖÃ¡£
	/* chroot if needed */
    ...
    //¸ù¾ÝÉè¶¨Öµ¶Ô½ø³Ì½øÐÐfork¡£ÉèÖÃ²¢·¢½ø³ÌÊý¡£
	if (global.mode & (MODE_DAEMON | MODE_SYSTEMD)) {
       ...
       //²¿·Öfork³öÀ´µÄ½ø³ÌÐèÒªÏÞÖÆÆä×ÊÔ´¡£¾ßÌåµÄ»¹Ã»ÓÐ¿´¡£
#ifdef USE_CPU_AFFINITY
        ...
        //ÐèÒªÔÚÄ³Ð©½ø³ÌÖÐÐ¶ÔØ²¿·Ö¼àÌý¶Ë¿ÚºÍÐ­Òé¡£ÕâÊÇÎªÊ²Ã´å
		...
		//ÓÐÐ©pollerÔÚforkÖ®ºó»á³öÎÊÌâ£¬ÐèÒªÖØÐÂ¼ÓÔØ¡£ÕâÀïÊÇµÚÒ»´Î¿´µ½pollerÕâ¸ö¶«Î÷¡£Õâ¸öÊÇÀí½âhaproxyµÄ¹Ø¼ü¡£
		
		fork_poller();
	}

    //´ò¿ªËùÓÐ×¢²áÐ­ÒéµÄ¼àÌýÕß£¬Õâ¸öºÍprotocol_bind_allÊÇÊ²Ã´¹ØÏµå
	protocol_enable_all();
	/*
	 * That's it : the central polling loop. Run until we stop.
	 */
	//HaproxyµÄÖ÷Ñ­»·
	//¹ØÓÚ¾ßÌåµÄÄÚÈÝÇë²ÎÕÕ¸Ãº¯ÊýµÄ·ÖÎö
	run_poll_loop();
